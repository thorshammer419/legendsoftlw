...
- [x] Bug fixes — universal draft persistence: (1) Lobby → ability scores screen: stats no longer reset — `handleSubmit` now calls `api.saveDraft` after `saveCharacter` so a fresh draft always exists on Lobby back-navigation; (2) CharacterCreate step 1 → New Campaign: character settings no longer lost — unmount cleanup effect calls `api.saveDraft` when component unmounts normally (Navbar back from step 1); `skipDraftSaveRef` flag prevents cleanup from firing on leave/cancel/submit; `campaignLoadedRef` prevents saving empty draft during loading. 4 new frontend tests (227 JS total).
