...
def mocks():
    with (
        ...
        patch("functions.membership.get_campaign_player") as mock_get_cp,
        patch("functions.membership.get_campaign_players") as mock_get_players,
        patch(f"{MODULE}.join_campaign") as mock_join_campaign,
...
async def test_open_lobby_campaign_returns_201(mocks):
    ...
    mocks["join_campaign"].assert_called_once()
...
async def test_already_member_returns_200_without_joining_again(mocks):
    ...
    mocks["join_campaign"].assert_not_called()
...
async def test_password_campaign_no_password_returns_403(mocks):
    ...
    mocks["get_campaign"].return_value = _open_campaign()
    mocks["join_campaign"].side_effect = DomainError("Password required", 403)
...
async def test_password_campaign_wrong_password_returns_403(mocks):
    ...
    from functions.domain import DomainError
    mocks["get_campaign"].return_value = _open_campaign()
    mocks["join_campaign"].side_effect = DomainError("Incorrect password", 403)
...
async def test_wrong_invite_token_returns_403(mocks):
    ...
    from functions.domain import DomainError
    mocks["get_campaign"].return_value = _open_campaign()
    mocks["join_campaign"].side_effect = DomainError("Invalid invite link", 403)
...
async def test_joining_active_campaign_enqueues_player_join(mocks):
    ...
    mocks["join_campaign"].return_value = {"campaign_was_active": True}
